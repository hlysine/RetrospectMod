...
public class VolatileEnergyPower extends AbstractPower implements CloneablePowerInterface, EnergySourceChangedSubscriber {

    public static final String POWER_ID = RetrospectMod.makeID(VolatileEnergyPower.class.getSimpleName());
    ...
    public VolatileEnergyPower(final AbstractCreature owner, int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public void atStartOfTurn() {
        super.atStartOfTurn();
        int decayAmount = (int) Math.ceil(this.amount / 2d);
        if (this.amount <= decayAmount) {
            addToBot(new RemoveSpecificPowerAction(this.owner, this.owner, this));
        } else {
            addToBot(new ReducePowerAction(this.owner, this.owner, this, decayAmount));
        }
    }

    @Override
    public void onEnergySourceChanged() {
        if (EnergySwitch.getCurrentSource() == EnergySwitch.EnergySource.VOLATILE) {
            flash();
            EnergyPanel.setEnergy(this.amount);
    ...
    public AbstractPower makeCopy() {
        return new VolatileEnergyPower(owner, amount);
    }
}
